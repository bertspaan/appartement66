...
text(55,30,'Tussenvorm: 30 cm naast de voordeur',38)
...
scale=76
# Allow 30 cm to the near face of the 10 cm partition, measured along the exterior wall.
end_proposal=-.54
for _ in range(10):
 end_proposal=-.84+.30+.05*hypot(5.76,end_proposal-.95)/5.76
for panel,endz in enumerate([-.81,end_proposal]):
 ...
 text(ox,140,'HUIDIG · 17°' if panel==0 else 'TUSSENVORM · 14°',26)
 ...
 cursor=0
 for start,end in [(1.88,2.73),(3.40,4.25) if panel else (3.10,3.95)]:
 ...
 if panel:
  c=along(3.825)
  line((c[0],c[1]+.12),(c[0]+.32,c[1]+.65),'#8b5879',2)
  label((c[0]-.55,c[1]+.68),'Tips deur →',18,'#8b5879')
 ...
 q=(5.02,-.84);r=(5.02,-.54 if panel else endz)
 line(q,r,'#b75c40',2)
 for v in [q,r]:line((v[0]-.08,v[1]),(v[0]+.08,v[1]),'#b75c40',2)
 label((4.85,.35 if panel else -.65),'30 cm' if panel else '3 cm*',18,'#9e4f37')
 text(ox,875,'Aansluiting vlak naast de voordeur' if not panel else '30 cm vrij tot de halzijde van de wand',23)
 text(ox,915,'Halwand haaks op slaapkamerwand' if not panel else 'Tips deur 30 cm naar rechts; opening blijft 85 cm',20,'#52636a')
 if panel:
  text(ox,952,'Slaapkamers: samen circa 0,9 m² kleiner',21,'#9e4f37')
text(65,1018,'Oranje: slaapkamerwand • turquoise: translucente halwand • stippellijn: huidige wand',21)
text(65,1054,'* Huidige maat tot hart wand. Voorstel: 30 cm vrij, met 10 cm wanddikte. 3D-model nog ongewijzigd.',19,'#657079')
out=Path(__file__).with_name('intermediate-diagonal.png');im.save(out);print(out)
